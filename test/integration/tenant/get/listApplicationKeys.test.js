...
describe("Testing list application key API", () => {
    ...
    it("Success - will return all tenant application key - id (admin)", (done) => {
        ...
        requester('/admin/tenant/application/key', 'get', params, (error, body) => {
            assert.ifError(error);
            assert.ok(body);
            assert.ok(body.data);
    ...
    it("Success - will return all tenant application key - no id", (done) => {
		let params = {
			qs: {
				appId: selectedTenant.applications[0].appId,
			},
		...
		requester('/tenant/application/key', 'get', params, (error, body) => {
			assert.ifError(error);
			assert.ok(body);
			assert.ok(body.data);
			assert.ok(Array.isArray(body.data));
	...
	it("Success - will return all tenant console application key", (done) => {
		let params = {
			qs: {
		...
		requester('/tenant/console/application/key', 'get', params, (error, body) => {
			assert.ifError(error);
			assert.ok(body);
			assert.ok(body.data);
			assert.ok(Array.isArray(body.data));
    ...
    it("Fail - will not return all tenant applicationkey config - no params", (done) => {
        let params = {};
        requester('/admin/tenant/application/key', 'get', params, (error, body) => {
            assert.ifError(error);
            assert.ok(body);
